...
const UserSidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  useEffect(() => {
    //  Fetching ProductsCategories
    (async () => {
      try {
        // need to create the productCategoris route at the backend
                  ...
                  {(handleClick, open) => {
                    return (
                      <React.Fragment>
                        <NavLink
                          to="#"
                          className={`group relative flex items-center gap-2.5 rounded-sm px-4 py-2 font-medium dark:text-bodydark1 text-black duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                            (pathname === "/" || pathname.includes("tables")) &&
